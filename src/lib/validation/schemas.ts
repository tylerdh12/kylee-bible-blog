...
export const donationQuerySchema = z.object({
  take: z.string()
    ...
    .refine(val => val >= 1 && val <= 100, 'Take must be between 1 and 100'),
  page: z.string()
...
export const postQuerySchema = z.object({
  take: z.string()
    .regex(/^\d+$/, 'Take must be a positive integer')
    .transform(val => parseInt(val, 10))
    .refine(val => val >= 1 && val <= 50, 'Take must be between 1 and 50'),
  page: z.string()
    .regex(/^\d+$/, 'Page must be a positive integer')
    .transform(val => parseInt(val, 10))
    .refine(val => val >= 0, 'Page must be 0 or greater'),
...
export function createValidationErrorResponse(error: ZodError<any>) {
